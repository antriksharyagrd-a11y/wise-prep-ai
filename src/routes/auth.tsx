...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in or create your HireWise account to unlock daily coding challenges, AI mock interviews, and resume feedback." },
      { property: "og:title", content: "Sign in — HireWise" },
      { property: "og:description", content: "Sign in to your HireWise account." },
    ],
  }),
  component: AuthPage,
});
    ...
    <main className="relative min-h-screen circuit-bg flex items-center justify-center px-6 py-16">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-sm">
        ...
        <div className="glass rounded-2xl p-6 shadow-glow">
          ...
          <p className="mt-1 text-sm text-muted-foreground">{mode === "signin" ? "Sign in to continue your prep." : "Start your prep in under a minute."}</p>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <input value={password} onChange={(e) => setPassword(e.target.value)} required type="password" minLength={6} placeholder="Password" className="w-full h-10 rounded-md bg-input px-3 text-sm outline-none ring-1 ring-white/10 focus:ring-primary" />
